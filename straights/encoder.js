...
export class BitmaskEncoder {
    ...
    async encodeAsync(n, sets) {
        ...
        const { buffer, numberOfSets } = this.createUncompressedBitMaskBuffer(n, sets);
    ...
    createUncompressedBitMaskBuffer(n, sets) {
        // Convert sets to array for processing
    ...
    async decodeAsync(encoded, n) {
        ...
        if (controlByte !== CONTROL_BYTE_UNCOMPRESSED &&
            controlByte !== CONTROL_BYTE_COMPRESSED) {
 ...
 */
async function compressData(data, controlByte) {
 ...
 */
function encodeGridToUint8Array(grid) {
 ...
 * @returns Uint8Array with packed bits.
 */
export function encodeGridToBase64Url(grid) {
    const bytes = encodeGridToUint8Array(grid);
    return toBase64Url(bytes);
}
export function decodeGridFromBase64Url(base64url, gridSize) {
 ...
 */
function decodeGrid(bytes, gridSize) {

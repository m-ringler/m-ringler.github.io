...
export class JQueryFieldRenderer {
    ...
    getBackgroundColor(field) {
        ...
        if (field.mode === Str8ts.FieldModes.BLACK ||
    ...
    getTextColor(field) {
        const colors = this.colors;
        if (field.mode === Str8ts.FieldModes.BLACKKNOWN ||
    ...
    getElement(field) {
        ...
        if (result.length == 0) {
            throw new Error(`Element not found: this.${JQueryFieldRenderer.getSelector(field)}`);
        }
        if (result.length > 1) {
            throw new Error(`Multiple elements found: this.${JQueryFieldRenderer.getSelector(field)}`);
        }
        return result;
    }
